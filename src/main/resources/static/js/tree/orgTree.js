    ...
    oTree.prototype = {
            ...
            $.ajax({
                ...
                success: function (rt) {
                    if (rt.code ==200 ) {
                       ...
                       /* delete  rt.success;*/
                            allJson = rt;
                        filterDataList = _this.filterDataSloveList(rt);
                        if(_this.settings.all){
                            //全部开启
                            allLoadData = _this.dataSolve(rt);  //进行数据处理
                        };
                    }else{
                    }
                }
            ...
            _this.$element.on('click', '.js_show_party_selector', function(){
                ...
                layer.confirm(contentsHTML, {
                    ...
                    $.ajax({
                        ...
                        type: "POST",//方法类型
                        dataType: "json",//预期服务器返回的数据类型
                        url: "/sys/role/assginPermission",//url
                        data: {
                            "rid":roleid,
                            "ids[]": arr
                        },
        ...
        searchFuc: function(data){
            ...
            var sdom = '<div class="multiPickerDlg_search_wrapper">'
                ...
                + '<input type="text"id="memberSearchInput"class="qui_inputText ww_inputText ww_searchInput_text" placeholder="搜索权限"></span>'
                + '</div>'
                ...
                function fillNode(zNode, dataNode, presentName) {
                    dataNode.forEach(function(aDate){
                        var thisNode = {
                            id: aDate.id,
                            name: aDate.name,
                            spellName: 'achild_' + spellChange(aDate.name),
                            parentId: aDate.parent_id,
                            crumbs: presentName +'/'+ aDate.name,
                            children: []
                        };
                        var noChildNode = {
                            id: aDate.id,
                            name: aDate.name,
                            spellName: 'bchild_' + spellChange(aDate.name),
                            parentId: aDate.parent_id,
                            crumbs: presentName +'/'+ aDate.name,
                        };
